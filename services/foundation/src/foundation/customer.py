...
class CustomerConsentService:
    ...
    def record_consent(
        ...
        if (
            not all(required)
            or consent.decision not in {"granted", "withdrawn"}
        ...
        if (
            consent.replaces_consent_id is not None
            and (scope.tenant_id, consent.replaces_consent_id) not in self._consents
        ):
            raise CustomerValidationError(
                "replacement consent must reference an existing tenant consent"
            )
    ...
    def has_consent(self, scope: ScopeContext, customer_id: str, purpose: str) -> bool:
        ...
        facts = [
            fact
            for (tenant_id, _), fact in self._consents.items()
            if tenant_id == scope.tenant_id
            and fact.customer_id == customer_id
            and fact.purpose == purpose
        ]
        if not facts:
            return False
    ...
    def complete_privacy_request(
        ...
        if (
            request.request_type not in {"export", "deletion"}
            or not request.scope
            or not request.legal_evidence
            or outcome not in {"exported", "deleted", "rejected"}
        ):
            raise CustomerValidationError(
                "privacy requests require a valid type, scope, legal evidence, and outcome"
            )
    ...
    def post_value(
        ...
        if effect.reversal_of_effect_id is not None:
            original = self._value_effects.get(
                (scope.tenant_id, effect.reversal_of_effect_id)
            )
            if (
                original is None
                or original.customer_id != effect.customer_id
                or original.value_type != effect.value_type
                or effect.amount != -original.amount
            ):
                raise CustomerValidationError(
                    "reversal must link to and exactly offset its original effect"
                )
    ...
    def reverse_value(
        ...
        return self.post_value(principal_id, session_id, scope, reversal, trace_id)

    def balance(self, scope: ScopeContext, customer_id: str, value_type: str) -> Decimal:
        self._customer(scope, customer_id)
        amounts = (
            effect.amount
            for (tenant_id, _), effect in self._value_effects.items()
            if tenant_id == scope.tenant_id
            and effect.customer_id == customer_id
            and effect.value_type == value_type
        )
        return sum(amounts, Decimal())
    ...
    def _identity(
        ...
        if not history_id or customer_id == related_customer_id or occurred_at.tzinfo is None:
            raise CustomerValidationError(
                "identity history requires distinct customers and timezone-aware time"
            )
