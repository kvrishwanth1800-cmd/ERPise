...
class DurableCustomerConsentStore:
    ...
    def create_customer(self, customer: Customer, trace_id: str) -> None:
        self._commit(
            ...
            lambda cursor: cursor.execute(
                "INSERT INTO customers (tenant_id, customer_id) VALUES (%s, %s)",
                (customer.tenant_id, customer.customer_id),
            ),
        )

    def record_consent(
        self, tenant_id: str, consent: ConsentFact, trace_id: str
    ) -> None:
        self._commit(
            tenant_id,
            "ConsentChanged",
            ...
            lambda cursor: cursor.execute(
                """
                INSERT INTO consent_facts (
                    tenant_id, consent_id, customer_id, purpose, decision, version,
                    evidence, scope, occurred_at, replaces_consent_id
                ...
                (
                    ...
                    consent.scope,
                    consent.occurred_at,
                    consent.replaces_consent_id,
                ),
            ),
        )

    def complete_privacy_request(
        self, tenant_id: str, request: PrivacyRequest, trace_id: str
    ) -> None:
        ...
        self._commit(
            ...
            lambda cursor: cursor.execute(
                ...
                INSERT INTO privacy_requests (
                    tenant_id, request_id, customer_id, request_type, scope,
                    legal_evidence, outcome
    ...
    def post_value(
        self, tenant_id: str, effect: StoredValueEffect, trace_id: str
    ) -> None:
        self._commit(
            tenant_id,
            "StoredValueChanged",
            ...
            lambda cursor: cursor.execute(
                """
                INSERT INTO stored_value_effects (
                    tenant_id, effect_id, customer_id, value_type, amount, reason,
                    occurred_at, reversal_of_effect_id
    ...
    def balance(self, tenant_id: str, customer_id: str, value_type: str) -> object:
        ...
        with connection.cursor() as cursor:
            cursor.execute(
                ...
                FROM stored_value_effects
                WHERE tenant_id = %s AND customer_id = %s AND value_type = %s
                """,
