...
class FulfillmentService:
    ...
    def promise(
        ...
        if method == "delivery" and (
            not address or (scope.tenant_id, address) not in self._serviceable_addresses
        ):
    ...
    def fail(
        ...
        promise = self._get(scope, fulfillment_id)
        if promise.status not in {"promised", "assigned", "dispatched"} or outcome not in {
            "retry",
            "cancel_refund",
        }:
