...
class FulfillmentService:
    ...
    def promise(
        ...
        is_serviceable = address and (scope.tenant_id, address) in self._serviceable_addresses
        if method == "delivery" and not is_serviceable:
    ...
    def _validate_promise(
        ...
        identifiers_are_invalid = (
            not fulfillment_id
            or fulfillment_id in self._fulfillments
            or not order_id
            or not payment_id
        )
        if identifiers_are_invalid:
            raise FulfillmentStateError(
                "Fulfillment, order, and payment identifiers must be unique and non-empty."
            )
    ...
    def _record(
        ...
        self.outbox.append(
            FulfillmentChanged(promise.fulfillment_id, promise.tenant_id, result, trace_id)
        )
