...
class DurableWarehouseStore:
    ...
    def task(self, tenant_id: str, warehouse_id: str, task_id: str) -> WarehouseTask | None:
        ...
        return WarehouseTask(task_id, tenant_id, warehouse_id, task_type, tuple(TaskLine(item["line_id"], item["product_id"], Decimal(item["quantity"]), item["source_bin_id"], item["destination_bin_id"], item["allow_substitution"]) for item in details["lines"]), status, assignee_id, claimed_by, tuple(details["scanned_line_ids"]), tuple(TaskException(item["exception_id"], item["line_id"], item["kind"], Decimal(item["quantity"]), item["reason"], item["substitute_product_id"], date.fromisoformat(item["occurred_at"]).strftime("%Y-%m-%d")) for item in details["exceptions"]))  # type: ignore[arg-type]
    ...
    def _commit(self, tenant_id: str, event_type: str, aggregate_id: str, trace_id: str, write: Any) -> None:
        with self._connection.transaction():
            with self._connection.cursor() as cursor:
                write(cursor)
                self._outbox.enqueue(cursor, DurableEvent(f"{event_type}-{tenant_id}-{aggregate_id}-{trace_id}", tenant_id, event_type, "v1", trace_id, {"aggregate_id": aggregate_id}))
