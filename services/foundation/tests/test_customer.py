...
from foundation.customer import ConsentFact, CustomerConsentService, CustomerValidationError, PrivacyRequest, StoredValueEffect
...
def test_consent_retains_purpose_evidence_time_scope_and_withdrawal_controls_access() -> None:
    subject = service()
    customer(subject, "c1")
    subject.record_consent("agent", "session", scope(), ConsentFact("grant", "c1", "marketing", "granted", "v2", "signed-form", "email", NOW), "trace-1")
    assert subject.has_consent(scope(), "c1", "marketing")
    subject.record_consent("agent", "session", scope(), ConsentFact("withdraw", "c1", "marketing", "withdrawn", "v2", "customer-request", "email", NOW.replace(minute=1), "grant"), "trace-2")
...
def test_identity_history_preserves_merge_and_unmerge() -> None:
    ...
    assert subject.merge("agent", "session", scope(), "merge-1", "one", "two", NOW, "trace").action == "merged"
    assert subject.unmerge("agent", "session", scope(), "unmerge-1", "one", "two", NOW, "trace").action == "unmerged"
...
def test_value_correction_is_immutable_linked_reversal_and_is_idempotent() -> None:
    ...
    subject.reverse_value("agent", "session", scope(), "credit-1", "reverse-1", "correction", NOW, "trace")
    assert subject.balance(scope(), "c1", "store-credit") == Decimal()
    with pytest.raises(CustomerValidationError, match="immutable"):
        subject.post_value("agent", "session", scope(), StoredValueEffect("credit-1", "c1", "store-credit", Decimal("4"), "changed", NOW), "trace")
...
def test_privacy_outcomes_are_scoped_and_retain_legal_evidence() -> None:
    ...
    completed = subject.complete_privacy_request("agent", "session", scope(), request, "exported", "trace")
