...
def test_customer_facts_events_and_reversal_are_durable_and_tenant_scoped(
    ...
    store.record_consent(
        "tenant-a",
        ConsentFact("grant", "c1", "marketing", "granted", "v1", "signed", "email", NOW),
        "consent-trace",
    )
    store.complete_privacy_request(
        "tenant-a",
        PrivacyRequest("export", "c1", "export", "profile", "verified", "exported"),
        "privacy-trace",
    )
    store.post_value(
        "tenant-a",
        StoredValueEffect("credit", "c1", "credit", Decimal("10"), "goodwill", NOW),
        "value-trace",
    )
    store.post_value(
        "tenant-a",
        StoredValueEffect("reversal", "c1", "credit", Decimal("-10"), "correction", NOW, "credit"),
        "reverse-trace",
    )
