...
from integrated import audit, cases, catalog, customers, edge, events, orders, pos, projections, reports, sessions
...
ACTIONS = {
    "/api/session": "session.read", "/api/products": "products.read", "/api/reports/sales": "reports.read",
    "/api/orders": "orders.read", "/api/projections/orders": "orders.read", "/api/consent": "consent.write",
    "/api/consent/revoke": "consent.write", "/api/events/replay": "orders.write", "/api/edge": "orders.read",
    "/api/cases": "cases.write", "/api/pos/quote": "orders.write", "/api/pos/shifts/open": "orders.write",
    "/api/pos/shifts/close": "orders.write", "/api/pos/sales": "orders.write", "/api/pos/returns": "orders.write",
}
...
class GovernedHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        ...
        try:
            ...
            elif self.path == "/api/pos/quote":
                result = pos.quote(tenant, cast(list[dict[str, Any]], data.get("lines", []))); action = "pos.quoted"
            elif self.path == "/api/pos/shifts/open":
                result = pos.open_shift(tenant, user, int(data.get("opening_float_cents", 0))); action = "pos.shift_opened"
            elif self.path == "/api/pos/shifts/close":
                result = pos.close_shift(tenant, str(data.get("shift_id", "")), int(data.get("declared_cash_cents", -1))); action = "pos.shift_closed"
            elif self.path == "/api/pos/sales":
                result = pos.complete_cash_sale(tenant, user, str(data.get("shift_id", "")), cast(list[dict[str, Any]], data.get("lines", [])), int(data.get("tendered_cents", -1)), self.headers.get("Idempotency-Key", "")); action = "pos.sale_completed"
            elif self.path.startswith("/api/pos/returns/"):
                result = pos.return_sale(tenant, self.path.rsplit("/", 1)[-1]); action = "pos.sale_returned"
            ...
            subject_id = result.get("sale_id", result.get("shift_id", result.get("case_id", result.get("reservation_id", result.get("order_id", result.get("customer_id", ""))))))
            audit.record(tenant, user, action, str(subject_id), self.trace_id())
            created = self.path in {"/api/orders", "/api/cases", "/api/edge/reserve", "/api/pos/shifts/open", "/api/pos/sales"}
            self.respond(result, HTTPStatus.CREATED if created else HTTPStatus.OK)
    ...
    def logout(self) -> None:
        ...
        if session is not None:
            AUTHORIZER.revoke(session); sessions.revoke(session["session_id"])
    ...
    def require_session(self) -> dict[str, str] | None:
        ...
        action = "orders.write" if (self.path == "/api/orders" or self.path.startswith("/api/pos/")) and self.command == "POST" else ACTIONS.get("/api/edge" if self.path.startswith("/api/edge") else "/api/cases" if self.path.startswith("/api/cases") else self.path)
    ...
    def session(self) -> dict[str, str] | None:
        cookie = SimpleCookie(self.headers.get("Cookie")); value = cookie.get("erpise_session")
        return sessions.get(None if value is None else value.value)

    def payload(self) -> dict[str, Any]:
        length = int(self.headers.get("Content-Length", "0"))
        return {} if length == 0 else cast(dict[str, Any], json.loads(self.rfile.read(length).decode()))
