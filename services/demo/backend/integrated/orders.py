...
def create(tenant_id: str, customer_id: str, product_id: str, quantity: int, method: str,
    ...
    with connect() as connection, connection.cursor() as cursor:
        cursor.execute(
            """SELECT order_id, payment_id, fulfillment_status FROM demo_orders
            WHERE tenant_id = %s AND idempotency_key = %s""",
            (tenant_id, idempotency_key),
        )
        existing = cursor.fetchone()
        if existing is not None:
            return {"order_id": str(existing[0]), "payment_id": str(existing[1]),
                    "fulfillment_status": str(existing[2]), "idempotent": True}
        cursor.execute(
        ...
        cursor.execute(
            ...
            (tenant_id, order_id, customer_id, product_id, quantity, payment_id, idempotency_key),
        )
        payload = json.dumps({"order_id": order_id, "payment_id": payment_id,
                              "fulfillment_status": "reserved", "product_id": product_id,
                              "quantity": quantity, "price_cents": int(product[0]), "method": method})
        cursor.execute(
            """INSERT INTO demo_outbox (event_id, tenant_id, event_type, payload)
            VALUES (%s, %s, 'order.created.v1', %s::jsonb)""",
            (event_id, tenant_id, payload),
        )
    return {"order_id": order_id, "payment_id": payment_id, "fulfillment_status": "reserved",
            "idempotent": False}
...
def list_orders(tenant_id: str) -> list[dict[str, object]]:
    ...
    with connect() as connection, connection.cursor() as cursor:
        cursor.execute(
            """SELECT order_id, payment_id, fulfillment_status FROM demo_orders
            WHERE tenant_id = %s ORDER BY order_id""", (tenant_id,)
        )
        rows = cursor.fetchall()
    return [{"order_id": str(row[0]), "payment_id": str(row[1]),
             "fulfillment_status": str(row[2])} for row in rows]
