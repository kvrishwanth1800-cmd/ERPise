...
def create() -> dict[str, str]:
    ...
    with connect() as connection, connection.cursor() as cursor:
        cursor.execute(
            ...
            VALUES (%s, %s, %s, %s, %s)""",
            (session_id, IDENTITY.tenant_id, IDENTITY.user_id, IDENTITY.role,
             datetime.now(UTC) + timedelta(hours=8)),
        )
    return {"session_id": session_id, "tenant_id": IDENTITY.tenant_id,
...
def get(session_id: str | None) -> dict[str, str] | None:
    ...
    if row is None:
        return None
    return {"session_id": session_id, "tenant_id": str(row[0]),
            "user_id": str(row[1]), "role": str(row[2])}
